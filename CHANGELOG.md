...
## 0.3.0 — 实验追溯、EOS 与离线评测

- checkpoint、LoRA adapter、运行 manifest 升级为版本 2，旧模型需要重新运行 06→10。
- 每步保存训练历史；执行失败与效果不达标分离，严格模式先保存后返回退出码 2。
- 独立 run 目录保存 base、语料和数据副本，以及每个 seed 的 random/full/lora 权重，支持搬迁与哈希校验。
- EOS 参与 SFT 监督和批次独立停止，新增指令模板推理。
- 增加 20 条 dev、40 条 test 和评分器反例；支持同义概念组、矛盾规则和失败原因。
- 开放预训练配置；自动生成 PNG/SVG，支持从历史记录跨配置绘图。
